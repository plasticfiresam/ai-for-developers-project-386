...
Интервал `[startTime, endTime)` на конкретную дату для выбранного `EventType`. Статус (только для дат внутри 14-дневного окна):

- `available` — нет пересечений с бронированиями;
- `booked` — есть пересечение;
- `past` — начало уже прошло (для сегодня).

Даты вне окна `[today, today + 13]` **не возвращаются API**: `/calendar` отдаёт только дни внутри окна; запрос `/days/{date}/slots` с датой вне окна → **400** без списка слотов.
...
| > `windowEnd` | Недоступна, вне окна; не включается в `/calendar`, `/days/{date}/slots` → 400 |
...
**Вход:** `eventTypeId`, `date` (обязательно внутри 14-дневного окна).
...
| День вне 14-дневного окна | Не возвращается API; в календаре гостя не отображается |
